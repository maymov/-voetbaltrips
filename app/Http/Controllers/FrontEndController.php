...
class FrontEndController extends JoshController
{
    ...
    public function getLogin()
    {
        ...
        // Show the login page
        return View::make('voetbaltrips_frontend.login');
    }
    ...
    public function postLogin(Request $request)
    {
        ...
        try {
            ...
            if (Sentinel::authenticate($request->only('email', 'password'), $request->get('remember-me', 0))) {
                ...
                /**
                 * Check that user have added anything in the cart
                 *
                 * If nothing go to my-account
                 *
                 * Otherwise redirect to cart payment confirm page.
                 *
                 */
                if($request->session()->has("match_data")) {
    ...
    {
        $user = Sentinel::getUser();
        return View::make('voetbaltrips_frontend.user_account', compact('user', 'countries'));
    }
    ...
    public function postRegister(UserRequest $request)
    {
        ...
        try {
            ...
            /**
             * Checking that user has selected any cart items
             * If the user selected any cart items then save the items to cart table
             */
            if($request->session()->has("match_data")) {
                return redirect(url("cart/payment"));
            }
