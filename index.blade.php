...
<section id="about" class="about">
        ...
        <div class="row">
            ...
            <div class="col-lg-12 text-center">
				...
				<input type="hidden" id="match_date" name="match_date" value="" />
                    ...
                    <span class="form-group col-lg-3 col-md-6">
                        <select name="city" class="form-control input-lg col-xs-4" id="city">
                            <option value="">{{Translater::getValue('form-title-choose-a-city')}}</option>
                            @foreach($city as $key=>$value)
                                <option {{ ((strtolower($city_uri) == strtolower($value['trans_name'])) ? 'selected="selected"':"") }} value="{{ $value['city_id'] }}">{{ $value['trans_name'] }}</option>
                            @endforeach
                        </select>
                    </span>
                    <span class="form-group col-lg-3 col-md-6">
                        <select name="club" id="club" class="form-control input-lg col-xs-4">
                            <option value="">{{Translater::getValue('form-label-choose-club')}}</option>
                            @foreach($club as $c_value)
                            <option value="{{ $c_value['id'] }}" {{ ((strtolower($club_uri) == strtolower($c_value['name']))? 'selected="selected"':"") }}>{{ $c_value['name'] }}</option>
                            @endforeach
                        </select>
                    </span>
                    <span class="form-group col-lg-3 col-md-6">
                        <select name="tournament" id="tournament" class="form-control input-lg col-xs-4">
                        <option value="">{{Translater::getValue('form-label-choose-tournament')}}</option>
                        @foreach($tournaments as $t_value)
                            <option value="{{ $t_value['id'] }}" {{ ((strtolower($tournament_uri) == strtolower($t_value['name'])) ? 'selected="selected"':"") }}> {{ $t_value['name'] }}</option>
                        @endforeach
                        </select>
                    </span>
    ...
    @section('footer_scripts')
        ...
        <script src="{{ asset('assets/voetbaltrips_frontend/js/homepage.js') }}"></script>
    @stop
</section>
<!-- Callout -->
