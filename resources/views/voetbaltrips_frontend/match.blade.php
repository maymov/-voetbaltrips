                                ...
                                <div class="product-price form-group categories">
                                    @foreach($match->competitionSeatingGet()->orderby("price", "desc")->get() as $x=>$seat)
                                        <div style="padding:20px 10px 12px 15px; margin-right: 15px; border-radius: 8px; background-color: {{ "#".$seat->seatingCategory->color}}" class="form-group">
                                            <input class="rdticket" type="radio" name="ticket" data-price="{{ addAdditionalPrice($seat->price) }}" value="{{ $seat->id }}" {{ (($cart_match['identifier'] == $seat->id)?'checked="checked"':"") }}>&nbsp;<label for="ticket"> {{ucfirst(Translater::getValue('label-category-small'))}} {{substr($seat->seatingCategory->name, -1)}}     &euro; + {{ number_format((addAdditionalPrice($seat->price) - addAdditionalPrice($match->competitionSeatingGet->min('price'))), 2) }} </label>
                                        </div>
                                    @endforeach
                                </div>
